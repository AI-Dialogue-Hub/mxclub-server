...
func (svc OrderService) ListTransferInfo(ctx jet.Ctx, params *api.PageParams) ([]*vo.TransferVO, int64, error) {
	...
	for _, transferPO := range transfers {
		...
		if e1 != nil {
			ctx.Logger().Errorf("FindByMemberNumber ERROR:%v e1:%v, p1:%v, p2: %v", e1, e2, p1, p2)
			return nil, 0, errors.New("查找失败")
		}
		var p2Name string
		if e2 != nil && p2 != nil {
